...
<div class="p-4 sm:ml-64 mt-8">
    <div class="flex justify-center">
        <div
            ...
            <form class="max-w-sm mx-auto pt-6" action="{{ route('operasi-rutin.store') }}" method="POST" id="yourForm">
                @csrf
                <div class="mb-5">
                    <label for="nim"
            ...
            if (nim.length === 9) { // Validasi NIM hanya jika panjangnya 9 karakter
                $.ajax({
                    ...
                    success: function(response) {
                        if (response) {
                            const tingkatDariKelas = response.kelas.charAt(
                                0); // Ambil inisial kelas untuk validasi tingkat
                            const sessionTingkat =
                                '{{ session('tingkat') }}'; // Ambil tingkat dari session

                            if (tingkatDariKelas === sessionTingkat) {
                                // Isi data jika tingkat sesuai
                                $('#nama_mahasiswa').val(response.nama);
                                $('#kelas').val(response.kelas);
                                $('#tingkat').val(response.tingkat);
                            } else {
                                // Kosongkan data jika tingkat tidak sesuai dengan session dan tampilkan pesan error
                                $('#nama_mahasiswa').val('');
                                $('#kelas').val('');
                                $('#tingkat').val('');
                                $('#nim').after(
                                    '<span id="nim-error" style="color: red; font-size: 12px;">Tingkat tidak sesuai</span>'
                                );
                            }
                        } else {
